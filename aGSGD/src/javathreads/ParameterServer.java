...
////            this.eng = this.masterThread.eng;
            ...
            curSGDparameters = masterThread.initSGDparameters;//get initial vals from master thread
            curSGDparameters.putVariables(eng);//put variable in local eng
...
//                }
                    ...
                    double []tmp = (double[])Wi;
                    System.out.println("Wi " + masterThread.getEp());
//                    for (int i = 0; i < 1; i++) {
...
//                System.out.println("******* " + dataSeq.size());             
    ...
    private void startSlaves(){
        System.out.println("staring slaves for PMS [" + Thread.currentThread().getId()+"]");
...
//            tgrad[i].runAsync();
            tgrad[i].start();
            System.out.println("slave started"); // [" + this.getId()+"].{" + tgrad[i].getId()+ "Aynch " + i + "} started.");
        }
    }
